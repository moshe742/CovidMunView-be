...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        for row in csv_data:
            ...
            agas_city_filtered = agas_city.filter(code=row[2]).first()
            try:
                agas_city_filtered.districts = row[3]
                agas_city_filtered.main_streets = row[4] if row[4] else 'unknown'
                logger.info(f'districts: {agas_city_filtered.districts}')
